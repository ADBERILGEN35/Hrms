...
public class JobAdManager implements JobAdService {
    ...
    public Result create(JobAdDto jobAdDto) {
        ...
        jobAd.setId(0);
        jobAd.setJobPosition(this.jobPositionDao.findById(jobAdDto.getJobPositionId()).get());
        jobAd.setEmployer(this.employerDao.findById(jobAdDto.getEmployerId()).get());
        jobAd.setDescription(jobAdDto.getDescription());
        jobAd.setCity(this.cityDao.findById(jobAdDto.getCityId()).get());
        ...
        jobAd.setWorkPlace(this.workPlaceDao.findById(jobAdDto.getWorkPlaceId()).get());
        jobAd.setWorkTime(this.workTimeDao.findById(jobAdDto.getWorkTimeId()).get());
    ...
    public Result setActiveAndConfirm(int jobAdId, int staffId) {
        try {
            ...
            JobAdActivation jobAdActivation = this.jobAdActivationDao.findById(jobAdId).get();
    ...
    public DataResult<JobAd> getByJobAdId(int id) {
        ...
        return new SuccessDataResult<JobAd>(this.jobAdDao.findById(id).get(), "Data listelendi");
    }
